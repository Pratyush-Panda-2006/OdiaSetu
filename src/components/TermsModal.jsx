...
export default function TermsModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-2xl bg-slate-900/95 border border-slate-800 rounded-2xl shadow-2xl flex flex-col max-h-[80vh] overflow-hidden transform transition-all duration-300 scale-100 z-10 animate-in fade-in zoom-in-95">
        
        {/* Header */}
        <div className="p-6 border-b border-slate-800/60 flex items-center justify-between bg-slate-950/40">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400">
              <Shield className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-xl font-serif font-bold text-white tracking-wide">Terms & Conditions</h2>
        ...
        {/* Content Body */}
        <div className="p-6 overflow-y-auto space-y-6 text-sm text-slate-300 leading-relaxed font-sans scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
          
          {/* Main Strict Banner */}
          <div className="relative p-5 rounded-xl bg-gradient-to-r from-indigo-950/40 to-slate-900/40 border border-indigo-500/30 overflow-hidden">
            ...
            <div className="flex gap-4">
              ...
              <div>
                <h4 className="font-semibold text-indigo-300 font-display mb-1 text-base">Ownership & Usage Restriction</h4>
        ...
        {/* Footer */}
        <div className="p-4 border-t border-slate-800/60 bg-slate-950/20 flex justify-between items-center gap-4">
          <span className="text-[10px] text-slate-500 font-display">© {new Date().getFullYear()} Pratyush Panda</span>
          <button 
            onClick={onClose}
            className="px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-white font-semibold text-xs tracking-wider uppercase transition-all duration-200 hover:shadow-lg hover:shadow-indigo-500/20 border border-indigo-500/20"
          >
            I Understand
          </button>
        </div>
